...
| Request parsing | The Worker calls the OpenAI Responses API with a strict JSON schema and returns the model name used. | The app uses a deterministic on-device parser and labels that result as a fallback. Funding-route suggestions remain local heuristics and are not provider eligibility decisions. |
| Donations | DonorsChoose projects open their official provider page. A locally created open need can use Stripe-hosted Checkout; the app records a receipt only after the Worker retrieves the session from Stripe and verifies `payment_status=paid`, amount, currency, and need ID. Sample projects can exercise the same card flow only with Stripe test-mode credentials and are excluded from real impact totals. | Checkout is disabled when the backend or callback scheme is missing. There is no fake payment success. |
...
- An OpenAI API project/key with access to the configured model. The Worker uses the [Responses API with Structured Outputs](https://developers.openai.com/api/docs/guides/structured-outputs) and defaults `OPENAI_MODEL` to `gpt-5.6-luna`.
...
`OPENAI_MODEL` and `CORS_ALLOWED_ORIGINS` are ordinary Worker variables in `wrangler.toml`; set exact browser origins for production. Native iOS requests normally have no `Origin`, and CORS is not authentication. Cloudflare's current guidance for secrets and deployment is in the [Workers secrets documentation](https://developers.cloudflare.com/workers/configuration/secrets/) and [Wrangler command reference](https://developers.cloudflare.com/workers/wrangler/commands/workers/).
...
- When live AI is enabled, the classroom request and selected context are sent through the Worker to OpenAI with `store: false`. Publish an accurate privacy/retention notice and review under-18 requirements before launch.
...
- Sample requests are fictional. Their checkout sessions are restricted to Stripe test mode, appear as test receipts, and do not count toward real impact totals or classroom funding.
